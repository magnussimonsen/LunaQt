...
extern "C" __declspec(dllexport) char *eulpsolve(header *p[], int np, char *newr, char *ramend)
{
	start(newr,ramend); // required to initialize global variables newram and ramend
	...
	lpsolve.lpsolvecaller.argout = 
		(struct structargout *) calloc(1 + lpsolve.lpsolvecaller.nlhs, sizeof(struct structargout));
