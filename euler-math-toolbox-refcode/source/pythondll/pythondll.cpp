		...
		f_testkey ftestkey, f_eulerprint feulerprint)
{
	...
	Py_Initialize();
	Py_InitModule("eumat", EmbMethods);
	...
	{
		strcpy(errorline,"Could not initialize the output catcher.");
		error=1; return 0;
	}
...
char *py_result ()
{
	...
	if (!catcher)
	{
		strcpy(errorline,"Could not reset the output catcher.");
		error=1; return 0;
	}
	...
	else 
	{
		...
		if (!out)
		{
			strcpy(errorline,"Could not get the output string.");
...
extern "C" __declspec(dllexport) char *py_run_file (char *filename)
{
	...
	PyObject* file = PyFile_FromString(filename,"r");
	if (!file) 
	{
		sprintf(errorline,"Could not open %s",filename);
		error=1; return 0;
	}

	if (PyRun_SimpleFile(PyFile_AsFile(file),filename)==-1) 
	{
		PyErr_Print();
...
double getdouble (PyObject *item)
{
	...
	if (PyInt_Check(item)) return PyInt_AsLong(item);
...
int pythontoeuler (PyObject *item, char *name, char **xnewram, char *xramend)
{
	...
	{
		new_string(PyString_AsString(item),"");
	}
	else if (PyInt_Check(item))
	{
		new_real((double)PyInt_AsLong(item),"");
	}
...
PyObject *eulertopython (header *hd)
{
	...
	{
		double x=*realof(hd);
		if ((long)x==x) return PyInt_FromLong((long)x);
	...
	else if (hd->type==s_string)
	{
		char *s=stringof(hd);
		if (strncmp(s,"py$",3))
			return PyString_FromString(stringof(hd));
		else
		{	
	...
	else if (hd->type==s_smatrix)
	{
		...
		while (*p!=1)
		{
			PyObject *item=PyString_FromString(p);
	...
	else if (hd->type==s_matrix && dimsof(hd)->r==1)
	{
		...
		{
			PyObject *item;
			if ((long)(*x)==(*x)) item=PyInt_FromLong((long)(*x++));
